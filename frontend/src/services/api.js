...
export const transactionApi = {
  ...
  getMonthlyReport: (walletId, months = 12) =>
    api.get(
      `/api/transactions/monthly-report?walletId=${walletId}&months=${months}`,
    ),
};
...
export const userApi = {
  ...
  lookupRecipient: (email) =>
    api.get(`/api/users/lookup?email=${encodeURIComponent(email)}`),
};
...
export const adminApi = {
  ...
  verifyWalletBalance: (walletId) =>
    api.get(`/api/admin/wallets/${walletId}/verify`),
  reconcileWallet: (walletId) =>
    api.post(`/api/admin/wallets/${walletId}/reconcile`),
};
